...
func (s *Scheduler) claimReadyDrops(ctx context.Context, now time.Time) {
	...
	for _, campaign := range snapshot.Inventory {
		...
		for _, drop := range campaign.Drops() {
			...
			ok, err := s.claimDrop(ctx, drop)
			if err != nil {
				s.logger.Warn("认领掉宝失败", "campaign_id", campaign.ID, "drop_id", drop.ID, "error", err)
				continue
			}
			if ok {
				s.logger.Info("认领掉宝成功", "campaign_id", campaign.ID, "drop_id", drop.ID)
			}
		}
	}
}

func (s *Scheduler) claimDrop(ctx context.Context, drop *domain.TimedDrop) (bool, error) {
	if drop == nil || drop.IsClaimed || drop.ClaimID == "" {
		return false, nil
	}

	operation, err := gql.MustLookup(gql.OperationClaimDrop).WithVariables(map[string]any{
		"input": map[string]any{
			"dropInstanceID": drop.ClaimID,
		},
	})
	if err != nil {
...
func (s *Scheduler) resolveProgress(ctx context.Context, channel domain.Channel) error {
	...
	if drop, currentMinutes, ok, err := s.fetchCurrentDrop(ctx, channel.ID); err != nil {
		s.logger.Warn("CurrentDrop 查询失败，回退本地补分钟", "channel_id", channel.ID, "error", err)
	} else if ok && drop != nil && drop.CanEarn(now, &channel, settings.EnableBadgesEmotes, false) {
		if drop.UpdateMinutes(currentMinutes) {
			s.recordProgress(now)
		}
		return nil
	}

	activeCampaign := s.ActiveCampaign(&channel)
	if activeCampaign == nil {
		return nil
	}

	reachedLimit := activeCampaign.BumpMinutes(now, &channel, settings.EnableBadgesEmotes, false)
	s.recordProgress(now)
	if reachedLimit {
		s.ChangeState(StateChannelSwitch)
	}
...
func (s *Scheduler) fetchCurrentDrop(ctx context.Context, channelID int64) (*domain.TimedDrop, int, bool, error) {
	...
	if err != nil {
		return nil, 0, false, fmt.Errorf("构造 CurrentDrop 请求失败: %w", err)
	}
	...
	snapshot := s.snapshotCopy()
	drop := snapshot.Drops[dropID]
	return drop, int(int64Value(session, "currentMinutesWatched")), true, nil
}
